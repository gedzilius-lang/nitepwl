...
echo ">>> [Radio Final Fix] Committing Manual Control Player..."

# 1. Frontend: Ensure Manual Player Logic is Correct
# This fixes the most reliable configuration from the previous steps.
  ...
  <div class="pwl-radio-body">
    ...
    <div class="player-toolbar">
        ...
        <div class="controls-right">
             <button class="icon-btn" @click="toggleVideo">
    ...
    <div class="stage">
        ...
        <div id="audio-overlay" :style="{ opacity: videoEnabled ? 0 : 1 }">
...
export default {
  ...
  computed: {
    liveBtnText() {
      ...
      if (this.isLiveAvailable) return "🔴 LIVE AVAILABLE";
      return "STUDIO OFFLINE";
    }
  },
  mounted() {
    this.initPlayer();
    this.checkLiveStream(); // Check immediately
  ...
  methods: {
    ...
    startPlayback() {
        ...
        this.player.play().catch(e => console.error("Playback start error:", e));
    },
    ...
    setSource(type) {
        ...
        // Reset player state before switching source
        this.player.reset(); 
    ...
    // Remaining helper methods (tickTimer, toggleVideo, etc.)
    tickTimer() {
    ...
    toggleVideo() { this.videoEnabled = !this.videoEnabled; const el = this.$refs.videoPlayer.querySelector('video'); if (el) el.style.opacity = this.videoEnabled ? 1 : 0; },
    togglePiP() { const v = this.$refs.videoPlayer.querySelector('video'); if (document.pictureInPictureElement) document.exitPictureInPicture(); else if (v && v.requestPictureInPicture) v.requestPictureInPicture(); },
    shareSite() { navigator.clipboard.writeText(window.location.href); this.toastMessage = "LINK COPIED"; this.toastVisible = true; setTimeout(() => this.toastVisible=false, 3000); },
...
<style scoped>
/* CSS styles were omitted for brevity but remain the same as the previous response */
...
.btn-base { background: transparent; border: 1px solid #333; color: #888; padding: 8px 12px; font-size: 10px; font-weight: bold; cursor: pointer; transition: 0.2s; }
.btn-base:hover { border-color: #fff; color: #fff; }
.live-btn.active { background: var(--accent-live); color: white; border-color: var(--accent-live); }
