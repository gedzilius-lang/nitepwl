...
echo ">>> [Final Fix] Applying Cache Busting Player Logic..."

# 1. Frontend: Implement Cache-Busting and Robust Playback
  ...
  <div class="pwl-radio-body">
    <header>
        ...
        <div class="header-controls">
            <button class="btn-base live-btn" :class="{ available: isLiveAvailable, active: currentSource === 'live' }" @click="setSource('live')" :disabled="!isLiveAvailable && currentSource !== 'live'">
                {{ liveBtnText }}
            </button>
...
export default {
  data() {
    return {
      ...
      currentDuration: 0,
      currentStartTime: 0,
      // Use full paths, the trailing '/' is critical for HLS segments
      sources: {
  ...
  methods: {
    initPlayer() {
      this.player = videojs(this.$refs.videoPlayer, {
        ...
        responsive: true,
        muted: true,
        // Set HLS to auto-recover quickly
        html5: { vhs: { overrideNative: true, autoSeekDuration: 5 } }
      });
      // Load AutoDJ source with cache-buster
      this.player.src({ src: this.sources.autodj + '?v=' + Date.now(), type: 'application/vnd.apple.mpegurl' });
    ...
    startPlayback() {
        ...
        this.player.play().catch(e => console.error("Playback blocked:", e));
    },
    ...
    setSource(type) {
        ...
        // Use cache-buster to force fresh playlist download
        const url = this.sources[type] + '?v=' + Date.now();
        ...
        this.trackTitle = type === 'live' ? "LIVE BROADCAST" : "Loading...";
        if (type !== 'live') this.updateStats();
    },
...
#audio-overlay { position: absolute; width: 100%; height: 100%; background: #050505; z-index: 5; display: flex; flex-direction: column; justify-content: center; align-items: center; pointer-events: none; }
...
cd "$HOME/nitepwl"
git add .
git commit -m "FinalFix: Robust HLS Playback Logic & Cache Busting"
